...
void bx_object_contact_store(MYSQL * mysql, BXObjectContact * contact)
{
    ...
    bx_database_add_param_int32(query, ":contact_type_id", &contact->contact_type_id);
    ...
    bx_database_add_param_int32(query, ":salutation_form", &contact->salutation_form);

    int64_t * contact_group_ids = bx_int_string_array_to_int_array(contact->contact_groupd_ids.value);
...
void * bx_object_contact_decode(void * jroot)
{
    ...
    contact->contact_type_id =           bx_object_get_json_uint(object, "contact_type_id", hashState);
    contact->salutation_id =             bx_object_get_json_uint(object, "salutation_id", hashState);
    contact->country_id =                bx_object_get_json_uint(object, "country_id", hashState);
    ...
    contact->salutation_form =           bx_object_get_json_uint(object, "salutation_form", hashState);
    contact->language_id =               bx_object_get_json_uint(object, "language_id", hashState);
    ...
    contact->updated_at =                bx_object_get_json_string(object, "updated_at", hashState);
    contact->city =                      bx_object_get_json_string(object, "city", hashState);
    contact->mail =                      bx_object_get_json_string(object, "mail", hashState);
    contact->mail_second =               bx_object_get_json_string(object, "mail_second", hashState);
    contact->phone_fixed =               bx_object_get_json_string(object, "phone_fixed", hashState);
    contact->phone_fixed_second =        bx_object_get_json_string(object, "phone_fixed_second", hashState);
    contact->phone_mobile =              bx_object_get_json_string(object, "phone_mobile", hashState);
    contact->fax =                       bx_object_get_json_string(object, "fax", hashState);
    contact->url =                       bx_object_get_json_string(object, "url", hashState);
    contact->skype_name =                bx_object_get_json_string(object, "skype_name", hashState);
    contact->remarks =                   bx_object_get_json_string(object, "remarks", hashState);
    contact->contact_groupd_ids =        bx_object_get_json_string(object, "contact_group_ids", hashState);
    contact->contact_branch_ids =        bx_object_get_json_string(object, "contact_branch_ids", hashState);
    contact->profile_image =             bx_object_get_json_string(object, "profile_image", hashState);
...
void bx_object_contact_dump(void * data)
{
    ...
    _bx_dump_print_title("### DUMP CONTACT '%s' CS:%lx ###", contact->nr.value, contact->checksum);
...
static void contact_group_sync(bXill * app, BXObjectContact * contact)
{
    ...
    uint64_t * group_ids = (uint64_t *)bx_int_string_array_to_int_array(contact->contact_groupd_ids.value);
...
bool bx_contact_sync_item(bXill * app, BXGeneric * item)
{
    ...
    if (query->results == NULL || query->results->column_count == 0) {
        ...
        query = bx_database_new_query(
            ...
            "phone_mobile, fax, url, skype_name, remarks, updated_at, profile_image, language_id,"
            "_checksum, _last_updated"
            ...
            ":fax, :url, :skype_name, :remarks, :updated_at, :profile_image, :language_id, "
            ":_checksum, :_last_updated"
            ");"
        );
    } else {
    ...
    bx_database_add_bxtype(query, ":contact_type_id", (BXGeneric *)&contact->contact_type_id);
    bx_database_add_bxtype(query, ":salutation_id", (BXGeneric *)&contact->salutation_id);
    ...
    bx_database_add_bxtype(query, ":salutation_form", (BXGeneric *)&contact->salutation_form);
    bx_database_add_bxtype(query, ":language_id", (BXGeneric *)&contact->language_id);
    ...
    bx_database_add_bxtype(query, ":mail_second", (BXGeneric *)&contact->mail_second);
    bx_database_add_bxtype(query, ":mail", (BXGeneric *)&contact->mail);
    bx_database_add_bxtype(query, ":phone_fixed_second", (BXGeneric *)&contact->phone_fixed_second);
    bx_database_add_bxtype(query, ":phone_fixed", (BXGeneric *)&contact->phone_fixed);
    bx_database_add_bxtype(query, ":phone_mobile", (BXGeneric *)&contact->phone_mobile);
    bx_database_add_bxtype(query, ":fax", (BXGeneric *)&contact->fax);
    bx_database_add_bxtype(query, ":url", (BXGeneric *)&contact->url);
    bx_database_add_bxtype(query, ":skype_name", (BXGeneric *)&contact->skype_name);
    bx_database_add_bxtype(query, ":remarks", (BXGeneric *)&contact->remarks);
    bx_database_add_bxtype(query, ":updated_at", (BXGeneric *)&contact->updated_at);
    bx_database_add_bxtype(query, ":profile_image", (BXGeneric *)&contact->profile_image);

    bx_database_add_param_uint64(query, ":_checksum", &contact->checksum);
    bx_database_add_param_uint64(query, ":_last_updated", &now);
    bx_database_add_param_char(query, ":country", (void *)bx_country_list_get_code(contact->country_id.value), 2);
    ...
    uint64_t * branch_ids = (uint64_t *)bx_int_string_array_to_int_array(contact->contact_branch_ids.value);
    if (branch_ids != NULL) {
        free(branch_ids);
    }
